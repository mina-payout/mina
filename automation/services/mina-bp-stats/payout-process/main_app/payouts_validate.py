...
def main(epoch_no, do_send_email):
    ...
    if not staking_df.empty and db_staus >= 0:
        ...
        for row in delegation_record_df.itertuples():
            ...
            filter_validation_record_df = staking_df.loc[
                (staking_df['pk'] == pub_key) & (staking_df['delegate'] == delegate_pub_key)]
            if not filter_validation_record_df.empty:
                ...
                current_acc_df = validation_record_df.query(
                    'provider_pub_key==@pub_key and global_slot_since_genesis >=@start_slot and global_slot_since_genesis<=@end_slot ')
                ...
                payouts_rows.append(
                    [pub_key, winner_pub_key, payout_amount, total_pay_received, balance_this_epoch, new_payout_balance,
                     epoch_no, start_slot, end_slot])
                winner_match = False
        ...
        email_df = pd.DataFrame(email_rows,
                                columns=["provider_pub_key", "winner_pub_key", "payout_amount", "payout_received"])
        ...
        payout_summary_df = pd.DataFrame(payouts_rows,
                                         columns=['provider_pub_key', 'winner_pub_key', 'payout_amount',
                                                  'payout_received', 'balance_this_epoch', 'payout_balance', 'epoch_no',
                                                  'start_slot', 'end_slot'])
        payout_summary_df = payout_summary_df.rename(
            columns={'payout_amount': 'payout_obligation', 'payout_balance': 'balance_cumulative'})
