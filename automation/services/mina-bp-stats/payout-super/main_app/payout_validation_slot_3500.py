...
def read_payout_summary_for_epoch(epoch):
    ...
    blocks_won, burn_amount, payout_balance, burn_balance from payout_summary where epoch=%s;"""
    ...
    try:
        ...
        summary_df = pd.DataFrame(blocks_produced_list,
                            columns=['provider_key', 'bp_key', 'blocks_produced', 'payout_amount', 
                                'blocks_won', 'burn_amount', 'payout_balance', 'burn_balance'])
...
def get_record_for_validation(return_wallets, start_slot, end_slot):
    ...
    SELECT    amount/power(10,9) as total_pay, pk.value as receiver,
    ...
    inner join user_commands AS uc on uc.id = buc.user_command_id
       and uc.type='payment' and status <>'failed'
...
def main(epoch):
    ...
    burn_return_wallet_key = 'B62qiburnzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzmp7r7UN6X'
    ...
    result = pd.DataFrame(columns=('delegating_wallet_name', 'delegating_wallet_key', 'return_wallet_key', 'bp_key', 'mds_hash', 'bp_email',
              'payout_amount', 'payout_received', 'payout_memo', 'burn_amount', 'burn_received', 'burn_memo' ))
    ...
    for index, row in super_df.iterrows():
        ...
        burn_received, burn_memo = find_return_amount(payout_transactions_df, burn_return_wallet_key, source_wallets)
