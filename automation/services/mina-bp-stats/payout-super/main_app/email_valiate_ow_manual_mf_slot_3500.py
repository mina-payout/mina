...
def send_mail(epoch_id, delegate_record_df, debug_mode=False):
    ...
    f = open('main_app/templates/Email-2-Payout-Delinquent-v4.0.html', "r")
    html_text = f.read()
    ...
    for i, row in payouts_df.iterrows():    
        ...
        if 1==1: #if 'Treasury' in wallet_name: 
            ...
            subject = f"""URGENT: Mina Foundation Delegation Program Missing Payment - Epoch {epoch_id}"""
            ...
            if debug_mode:
                break
    logger.info("Delinquent: epoch number: {0}, emails sent: {1}".format(epoch_id,count))
...
if __name__ == "__main__":
    ...
    debug_mode = True
    debug_mode = False
    df = pd.read_csv('summary_files/delinquent_summary_{0}_1.csv'.format(epoch), index_col=0)
