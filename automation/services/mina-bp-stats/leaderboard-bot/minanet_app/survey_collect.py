        ...
        if ten_min_add_time_var > script_end_time_var:
            ...
            try:
                ...
                if not point_record_df.empty:
                    ...
                    max_block_height = point_record_df['nodedata_blockheight'].max()
                    min_block_height = point_record_df['nodedata_blockheight'].min()
                    logger.info(
                        'min block height{0} and max block height{1} '.format(min_block_height, max_block_height))
                    distinct_blockchain_height = point_record_df['nodedata_blockheight'].nunique()
                    if distinct_blockchain_height == 2:
                        point_record_df = point_record_df.drop(
                            point_record_df[(point_record_df['nodedata_blockheight'] == min_block_height)].index)
                    elif distinct_blockchain_height > 2:
                        # filtered dataframe removed max and min height
                        point_record_df = point_record_df.drop(
                            point_record_df[
                                (point_record_df['nodedata_blockheight'] == min_block_height) | 
                                    (point_record_df['nodedata_blockheight']== max_block_height)].index)

                    most_common_state_hash = point_record_df['nodedata_block_statehash'].value_counts().idxmax()

                    point_record_df['amount'] = np.where(
                        point_record_df['nodedata_block_statehash'] == str(most_common_state_hash), 1, -1)
